...
void initPomdpSolve(PomdpSolveParams param) {
        ...
        initializePomdp(param->param_filename, param->impossible_obs_epsilon);
        ...
        if (param->override_discount >= 0.0) gDiscount = param->override_discount;
        ...
        if (param->initial_policy_filename[0] != NULL_CHAR) {
                if ((param->initial_policy = readAlphaList(param->initial_policy_filename, -1)) == NULL) {
                        sprintf(msg, "Cannot open initial policy file name: %s.\n\t(Using default)",
...
/**********************************************************************/
void endPomdpSolve(PomdpSolveParams param, AlphaList solution) {
        /*
        ...
        */
        ...
        Assert(param != NULL && solution != NULL, "Bad (NULL) parameters.");
        ...
        if (gVerbose[V_POMDP_SOLVE]) {
                fprintf(param->report_file, "The solution to the (in)finite horizon is:\n");
                displayAlphaList(param->report_file, solution);
        } else {
                fprintf(param->report_file, "++++++++++++++++++++++++++++++++++++++++\n");
                fprintf(param->report_file, "Solution found.  See file:\n\t%s\n\t%s\n", param->alpha_filename,
                        param->pg_filename);
                fprintf(param->report_file, "++++++++++++++++++++++++++++++++++++++++\n");
        }
...
AlphaList getDefaultInitialPolicy() {
        ...
        */
        ...
        alpha = newAlpha();
        for (i = 0; i < gNumStates; i++) alpha[i] = 0.0;
...
/**********************************************************************/
void handleSaveAll(AlphaList list, PomdpSolveParams param) {
        /*
        ...
        */
        ...
        if (param->vi_variation != ZlzSpeedup) APG_writePolicyGraph(list, pg_filename);
...
/**********************************************************************/
double weakBound(AlphaList cur_alpha_list, AlphaList prev_alpha_list, double delta) {
        /*
        ...
        */
        ...
        Assert(prev_alpha_list != NULL && cur_alpha_list != NULL, "Bad (NULL) parameter(s).");
        ...
        while (p_x != NULL) {
                ...
                while (p_y != NULL) {
                        ...
                        if (max_s < min_p_y) min_p_y = max_s;

                        p_y = p_y->next;
                } /* while p_y */

                if (min_p_y > max_p_x) max_p_x = min_p_y;

                /* This is an optimization to exit the loop early */
                if (max_p_x > delta) return HUGE_VAL;
...
/**********************************************************************/
int meetStopCriteria(AlphaList prev_alpha_list, AlphaList cur_alpha_list, double *error, PomdpSolveParams param) {
        /*
        ...
        */
        ...
        /* Else select from exact stopping criteria setting. */
                ...
                switch (param->opts->stop_criteria) {
                        case POMDP_SOLVE_OPTS_Stop_Criteria_exact:
                                ...
                                if (sameAlphaList(prev_alpha_list, cur_alpha_list, param->alpha_epsilon)) {
                                        *error = 0.0;
                        ...
                        case POMDP_SOLVE_OPTS_Stop_Criteria_weak:
                                *error = weakBound(cur_alpha_list, prev_alpha_list, param->stop_delta);
                                break;

                        case POMDP_SOLVE_OPTS_Stop_Criteria_bellman:
                                *error = bellmanError(prev_alpha_list, cur_alpha_list, param);
                                break;
        ...
        if (*error <= param->stop_delta * (1.0 - gDiscount) / (2.0 * gDiscount))
                return TRUE;
        else
                return FALSE;
...
void setSolvePrecision(double epsilon, PomdpSolveParams param) {
        ...
        */
        ...
        if (param->q_purge_option == purge_epsilon_prune)
                ...
                param->impossible_obs_epsilon = Min(param->epsilon, DEFAULT_IMPOSSIBLE_OBS_EPSILON);
...
void doAdjustableEpsilonVariation(PomdpSolveParams param) {
        ...
        */
        ...
        Assert(param != NULL, "NULL parameters.");
        ...
        /* First log in the epsilon that was used for this epoch before
           making any changes. */
        recordEpochMaxEpsilon(param->cur_epoch, cur_epsilon, param->stat);

        /* If we have reached the end epsilon, then we don't have to worry
           about adjusting the epsilon at all. */
        if (cur_epsilon <= param->ending_epsilon) return;
        ...
        /* If there haven't been enough epochs to have enough history for
           ...
           bail out also. */
        if (param->cur_epoch < param->epoch_history_window_length) return;
        ...
        max_vects = 0;
        min_vects = 99999999;
        for (epoch = (param->cur_epoch - param->epoch_history_window_length + 1); epoch <= param->cur_epoch; epoch++) {
        ...
        if ((max_vects - min_vects) > param->epoch_history_window_delta) return;
        ...
        fprintf(param->report_file, ">>Adjusted epsilon to %.3e<<\n", cur_epsilon);
        ...
        */
        ...
        switch (param->vi_variation) {
                ...
                case FixedSolnSizeVi:
                        ...
                        fprintf(param->report_file, ">>Starting epsilon set to %.3e<<\n", param->starting_epsilon);
...
/**********************************************************************/
AlphaList improveByQ(AlphaList **projection, PomdpSolveParams param) {
        /*
      ...
      */
        ...
        Assert(projection != NULL && param != NULL, "Bad (NULL) parameters.");

        new_list = newAlphaList();

        for (a = 0; a < gNumActions; a++) {
                if (gVerbose[V_POMDP_SOLVE] == TRUE) getLpStats(param->stat, &start_lps, &start_constraints);
                ...
                if (gVerbose[V_POMDP_SOLVE] == TRUE) {
                        ...
                        fprintf(param->report_file, "Construct Q end: Q^%d size: %d  M: %d  Z: %d\n", a,
                                sizeAlphaList(cur_list), maxSizeAlphaLists(projection[a], gNumObservations),
                                gNumObservations);
                        fprintf(param->report_file, "\tLPs: %d\tConstraints: %d\n", end_lps - start_lps,
                                end_constraints - start_constraints);
        ...
        purgeAlphaList(new_list, param->q_purge_option, param);

        /* If we used epsilon pruning, then record the computed difference
           which was temporarily stored in the 'param' structure. */
        if ((param->stat != NULL) && (param->q_purge_option == purge_epsilon_prune))
                recordEpochMaxEpsilon(param->stat->cur_epoch, param->epsilon_diff_of_last_prune, param->stat);
...
/**********************************************************************/
AlphaList improveV(AlphaList prev_alpha_list, PomdpSolveParams param) {
        /*
        ...
        */
        ...
        Assert(prev_alpha_list != NULL && param != NULL, "Bad (NULL) parameters.");
...
void solvePomdp(PomdpSolveParams param) {
        ...
        */
        ...
        Assert(param != NULL, "Parameter structure is NULL.");
        ...
        if (param->save_all == TRUE) handleSaveAll(NULL, param);
        ...
        if (gVerbose[V_POMDP_SOLVE] == TRUE) {
                ...
                fprintf(param->report_file, "[Initial policy has %d vectors.]\n", sizeAlphaList(next_alpha_list));
        ...
        param->stat = newSolutionStats(param->report_file, param->stat_summary);
        ...
        while ((gInterrupt == FALSE) && (done == FALSE)) {
                ...
                if (prev_alpha_list != NULL) destroyAlphaList(prev_alpha_list);
                ...
                */
                ...
                if (meetStopCriteria(prev_alpha_list, next_alpha_list, &cur_error, param) == TRUE) done = TRUE;
                ...
                /* Stop the clock for this epoch and show some end of epoch
                   information */
                epochEndStats(param->stat, sizeAlphaList(next_alpha_list), cur_error);

                /* Check for finite horizon stopping condition. */
                if (param->cur_epoch == param->horizon) done = TRUE;
