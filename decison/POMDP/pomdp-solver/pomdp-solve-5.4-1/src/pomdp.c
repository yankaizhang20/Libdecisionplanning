...
void setPossibleObservations(double epsilon) {
        ...
        */
                        ...
                        for (cur_state = 0; cur_state < gNumStates; cur_state++)
                                for (j = P[a]->row_start[cur_state];
                                     j < P[a]->row_start[cur_state] + P[a]->row_length[cur_state]; j++)
                                        if (!Equal(getEntryMatrix(R[a], P[a]->col[j], z), 0.0, epsilon)) {
        ...
        for (a = 0; a < gNumActions; a++)
                Assert(gNumPossibleObservations[a] > 0, "Bad POMDP. No observations possible for some action.");

} /* setPossibleObservations */
/**********************************************************************/
void initializePomdp(char *filename, double obs_possible_epsilon) {
        /*
        ...
        */
        ...
        if ((filename == NULL) || (filename[0] == NULL_CHAR)) {
                sprintf(msg, "No parameter file specified (Use '%s' for options.)", CMD_ARG_HELP_SHORT);
                Abort(msg);
        }
        ...
        if (gProblemType != POMDP_problem_type) {
                sprintf(msg, "Parameter file is not a POMDP specification.");
                Abort(msg);
        }

        /* We'll use this stuff if the setPossibleObservations() routine is
           called. */
        gObservationPossible = (int **)XMALLOC(gNumActions * sizeof(*gObservationPossible));
        for (a = 0; a < gNumActions; a++)
                gObservationPossible[a] = (int *)XCALLOC(gNumObservations, sizeof(**gObservationPossible));

        gNumPossibleObservations = (int *)XCALLOC(gNumActions, sizeof(*gNumPossibleObservations));
...
void cleanUpPomdp() {
        ...
        */
        ...
        for (a = 0; a < gNumActions; a++) XFREE(gObservationPossible[a]);
...
double getAdjustedReward(int action, int state) {
        ...
        */
        ...
        if (gValueType == COST_value_type) reward *= -1.0;

        if ((gRequireNonNegativeRewards) && (gMinimumImmediateReward < 0.0)) reward -= gMinimumImmediateReward;
...
int valuesRequireScaling() {
        ...
        */
        return (((gRequireNonNegativeRewards) && (gMinimumImmediateReward < 0.0)) || (gValueType == COST_value_type));
...
double getValueScaleFactor(int num_updates) {
        ...
        */
        ...
        if ((gRequireNonNegativeRewards) && (gMinimumImmediateReward < 0.0)) {
                ...
                if (gDiscount == 1.0)
                        ...
                        scale_factor =
                                gMinimumImmediateReward * (1.0 - pow(gDiscount, num_updates - 1)) / (1.0 - gDiscount);

        } /* if need to adjust reward for non-negativity */

        if (gValueType == COST_value_type) scale_factor *= -1.0;
