...
class EudmPlannerVisualizer {
    public:
        EudmPlannerVisualizer(ros::NodeHandle nh, EudmManager* p_bp_manager, int ego_id) : nh_(nh), ego_id_(ego_id) {
        ...
        void PublishDataWithStamp(const ros::Time& stamp) { VisualizeForwardTrajectories(stamp); }
        ...
        void VisualizeForwardTrajectories(const ros::Time& stamp) {
                ...
                for (int i = 0; i < static_cast<int>(forward_trajs.size()); ++i) {
                        ...
                        for (const auto& v : forward_trajs[i]) {
                                ...
                                common::VisualizationUtil::GetRosMarkerCylinderUsingPoint(
                                        common::Point(pt), Vec3f(0.5, 0.5, 0.1), traj_color, 0, &point_marker);
                ...
                common::VisualizationUtil::FillHeaderIdInMarkerArray(stamp, std::string("map"),
                                                                     last_forward_trajs_marker_cnt_, &traj_list_marker);
