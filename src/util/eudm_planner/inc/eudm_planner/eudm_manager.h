...
class EudmManager {
    public:
        ...
        ErrorType Run(const decimal_t stamp, const std::shared_ptr<semantic_map_manager::SemanticMapManager>& map_ptr,
        ...
        std::shared_ptr<semantic_map_manager::SemanticMapManager> map() { return map_adapter_.map(); }

    private:
        decimal_t GetNearestFutureDecisionPoint(const decimal_t& stamp, const decimal_t& delta);

        bool IsTriggerAppropriate(const LateralBehavior& lat);

        ErrorType Prepare(const decimal_t stamp,
                          const std::shared_ptr<semantic_map_manager::SemanticMapManager>& map_ptr,
                          const planning::eudm::Task& task);

        ErrorType EvaluateReferenceVelocity(const planning::eudm::Task& task, decimal_t* ref_vel);

        bool GetReplanDesiredAction(const decimal_t current_time, DcpAction* desired_action);

        void SaveSnapshot(Snapshot* snapshot);

        ErrorType ReselectByContext(const decimal_t stamp, const Snapshot& snapshot, int* new_seq_id);

        void UpdateLaneChangeContextByTask(const decimal_t stamp, const planning::eudm::Task& task);

        ErrorType GenerateLaneChangeProposal(const decimal_t& stamp, const planning::eudm::Task& task);
