...
class DcpTree {
    public:
        ...
        enum class DcpLonAction { kMaintain = 0, kAccelerate, kDecelerate, MAX_COUNT = 3 };

        enum class DcpLatAction { kLaneKeeping = 0, kLaneChangeLeft, kLaneChangeRight, MAX_COUNT = 3 };
        ...
        struct DcpAction {
                ...
                friend std::ostream& operator<<(std::ostream& os, const DcpAction& action) {
                        os << "(lon: " << static_cast<int>(action.lon) << ", lat: " << static_cast<int>(action.lat)
                           << ", t: " << action.t << ")";
                        return os;
                }

                DcpAction() {}
                DcpAction(const DcpLonAction& lon_, const DcpLatAction& lat_, const decimal_t& t_)
                    : lon(lon_), lat(lat_), t(t_) {}
        };

        DcpTree(const int& tree_height, const decimal_t& layer_time);
        DcpTree(const int& tree_height, const decimal_t& layer_time, const decimal_t& last_layer_time);
        ~DcpTree() = default;

        void set_ongoing_action(const DcpAction& a) { ongoing_action_ = a; }

        std::vector<std::vector<DcpAction>> action_script() const { return action_script_; }
    ...
    private:
        ...
        std::vector<DcpAction> AppendActionSequence(const std::vector<DcpAction>& seq_in, const DcpAction& a,
                                                    const int& n) const;
