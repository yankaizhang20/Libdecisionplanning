...
class SscVisualizer {
    public:
        ...
        void VisualizeDataWithStamp(const ros::Time &stamp, const SscPlanner &planner);

    private:
        void VisualizeSscMap(const ros::Time &stamp, const SscMap *p_ssc_map);
        void VisualizeEgoVehicleInSscSpace(const ros::Time &stamp, const common::FsVehicle &fs_ego_vehicle);
        void VisualizeForwardTrajectoriesInSscSpace(const ros::Time &stamp,
                                                    const vec_E<vec_E<common::FsVehicle>> &trajs,
                                                    const SscMap *p_ssc_map);
        void VisualizeQpTrajs(const ros::Time &stamp, const vec_E<common::BezierSpline<5, 2>> &trajs);
        void VisualizeSurroundingVehicleTrajInSscSpace(
                ...
                const SscMap *p_ssc_map);
        void VisualizeCorridorsInSscSpace(const ros::Time &stamp, const vec_E<common::DrivingCorridor> corridor_vec,
                                          const SscMap *p_ssc_map);
