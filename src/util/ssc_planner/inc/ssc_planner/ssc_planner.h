...
class SscPlanner : public Planner {
    public:
        ...
        std::unordered_map<int, vec_E<common::FsVehicle>> sur_vehicle_trajs_fs() const { return sur_vehicle_trajs_fs_; }

        vec_E<std::unordered_map<int, vec_E<common::FsVehicle>>> surround_forward_trajs_fs() const {
                return surround_forward_trajs_fs_;
        };

        vec_E<vec_E<common::FsVehicle>> forward_trajs_fs() const { return forward_trajs_fs_; }

        vec_E<Vec2f> ego_vehicle_contour_fs() const { return fs_ego_vehicle_.vertices; }
        ...
        std::unique_ptr<FrenetTrajectory> trajectory() const {
                ...
                return std::unique_ptr<FrenetBezierTrajectory>(new FrenetBezierTrajectory(trajectory_));
        }
        ...
        common::FrenetState initial_frenet_state() const { return initial_frenet_state_; }
