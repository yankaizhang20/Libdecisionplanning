...
SscVisualizer::SscVisualizer(ros::NodeHandle nh, int node_id) : nh_(nh), node_id_(node_id) {
        ...
        std::string ssc_map_vis_topic =
                std::string("/vis/agent_") + std::to_string(node_id_) + std::string("/ssc/map_vis");
...
void SscVisualizer::VisualizeQpTrajs(const ros::Time &stamp, const vec_E<common::BezierSpline<5, 2>> &trajs) {
        ...
        for (int i = 0; i < static_cast<int>(trajs.size()); i++) {
                ...
                common::VisualizationUtil::FillScaleColorInMarker(Vec3f(0.2, 0.2, 0.2), common::cmap.at("magenta"),
                                                                  &traj_mk);
...
void SscVisualizer::VisualizeEgoVehicleInSscSpace(const ros::Time &stamp, const common::FsVehicle &fs_ego_vehicle) {
        ...
        common::VisualizationUtil::GetRosMarkerLineStripUsing2DofVecWithOffsetZ(contour, color, Vec3f(0.3, 0.3, 0.3),
                                                                                dt, 0, &ego_contour_marker);
        ...
        common::VisualizationUtil::GetRosMarkerSphereUsingPoint(
                Vec3f(fs_ego_vehicle.frenet_state.vec_s[0], fs_ego_vehicle.frenet_state.vec_dt[0], dt),
        ...
        for (int i = 0; i < static_cast<int>(trajs.size()); ++i) {
                ...
                for (int k = 0; k < static_cast<int>(trajs[i].size()); ++k) {
                        ...
                        common::ColorARGB color = common::GetJetColorByValue(
                                static_cast<decimal_t>(k), static_cast<decimal_t>(trajs[i].size() - 1), 0.0);
                        contour.push_back(contour.front());
                        ...
                        common::VisualizationUtil::GetRosMarkerSphereUsingPoint(
                                Vec3f(x, y, dt), common::cmap.at("cyan"), Vec3f(0.2, 0.2, 0.2), id_cnt++, &fs_mk);
...
void SscVisualizer::VisualizeSurroundingVehicleTrajInSscSpace(
        ...
        int num_markers = static_cast<int>(trajs_markers.markers.size());
        common::VisualizationUtil::FillHeaderIdInMarkerArray(stamp, std::string("ssc_map"),
                                                             last_sur_vehicle_traj_mk_cnt, &trajs_markers);
...
void SscVisualizer::VisualizeCorridorsInSscSpace(const ros::Time &stamp,
        ...
        for (const auto &corridor : corridor_vec) {
                ...
                for (const auto &driving_cube : corridor.cubes) {
                        ...
                        common::ColorARGB color =
                                common::GetJetColorByValue(cube_cnt, corridor.cubes.size() - 1, -kEPS);
                        for (const auto &seed : driving_cube.seeds) {
                                ...
                                common::VisualizationUtil::GetRosMarkerSphereUsingPoint(
                                        Vec3f(s_x, s_y, s_z - start_time_), color, Vec3f(0.3, 0.3, 0.3), id_cnt++,
                                        &seed_marker);
                        ...
                        p_ssc_map->p_3d_grid()->GetGlobalMetricUsingCoordOnSingleDim(driving_cube.cube.upper_bound[0],
                                                                                     0, &x_max);
                        p_ssc_map->p_3d_grid()->GetGlobalMetricUsingCoordOnSingleDim(driving_cube.cube.lower_bound[0],
                                                                                     0, &x_min);
                        p_ssc_map->p_3d_grid()->GetGlobalMetricUsingCoordOnSingleDim(driving_cube.cube.upper_bound[1],
                                                                                     1, &y_max);
                        p_ssc_map->p_3d_grid()->GetGlobalMetricUsingCoordOnSingleDim(driving_cube.cube.lower_bound[1],
                                                                                     1, &y_min);
                        p_ssc_map->p_3d_grid()->GetGlobalMetricUsingCoordOnSingleDim(driving_cube.cube.upper_bound[2],
                                                                                     2, &z_max);
                        p_ssc_map->p_3d_grid()->GetGlobalMetricUsingCoordOnSingleDim(driving_cube.cube.lower_bound[2],
                                                                                     2, &z_min);
        ...
        common::VisualizationUtil::FillHeaderIdInMarkerArray(ros::Time::now(), std::string("ssc_map"),
                                                             last_corridor_mk_cnt, &corridor_vec_marker);
