...
void SscMap::UpdateMapOrigin(const common::FrenetState &ori_fs) {
        ...
        map_origin[1] = -1 * (config_.map_size[1] - 1) * config_.map_resolution[1] / 2.0; // d
        ...
        for (const auto corridor : driving_corridor_vec_) {
                ...
                if (!corridor.is_valid) {
                        ...
                        for (int k = 0; k < static_cast<int>(corridor.cubes.size()); ++k) {
                                ...
                                p_3d_grid_->GetGlobalMetricUsingCoordOnSingleDim(corridor.cubes[k].cube.lower_bound[0],
                                                                                 0, &x_lb);
                                p_3d_grid_->GetGlobalMetricUsingCoordOnSingleDim(corridor.cubes[k].cube.upper_bound[0],
                                                                                 0, &x_ub);
                                p_3d_grid_->GetGlobalMetricUsingCoordOnSingleDim(corridor.cubes[k].cube.lower_bound[1],
                                                                                 1, &y_lb);
                                p_3d_grid_->GetGlobalMetricUsingCoordOnSingleDim(corridor.cubes[k].cube.upper_bound[1],
                                                                                 1, &y_ub);
                                p_3d_grid_->GetGlobalMetricUsingCoordOnSingleDim(corridor.cubes[k].cube.lower_bound[2],
                                                                                 2, &z_lb);
                                p_3d_grid_->GetGlobalMetricUsingCoordOnSingleDim(corridor.cubes[k].cube.upper_bound[2],
                                                                                 2, &z_ub);
        ...
        for (int i = 0; i < static_cast<int>(obs_grid_fs.size()); ++i) {
                ...
                for (int k = 0; k < config_.map_size[2]; ++k) {
                        std::array<decimal_t, 3> pt = {
                                {obs_grid_fs[i](0), obs_grid_fs[i](1), (double)k * config_.map_resolution[2]}};
