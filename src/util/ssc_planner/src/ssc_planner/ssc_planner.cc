...
ErrorType SscPlanner::Init(const std::string config_path) {
        ...
        LOG(INFO) << "[Ssc] -- low spd threshold: " << cfg_.planner_cfg().low_speed_threshold();
        LOG(INFO) << "[Ssc] -- weight_proximity: " << cfg_.planner_cfg().weight_proximity();
        ...
        map_cfg.kMinLongitudinalVel =
                std::max(cfg_.map_cfg().dyn_bounds().min_lon_vel(), cfg_.planner_cfg().velocity_singularity_eps());
...
ErrorType SscPlanner::RunOnce() {
        ...
        if (map_itf_->GetForwardTrajectories(&forward_behaviors_, &forward_trajs_, &surround_forward_trajs_) !=
            kSuccess) {
        ...
        for (int i = 0; i < num_behaviors; ++i) {
                ...
                //        timer_infl.toc());
                if (p_ssc_map_->ConstructCorridorUsingInitialTrajectory(p_ssc_map_->p_3d_grid(),
                                                                        forward_trajs_fs_[i]) != kSuccess) {
        ...
        if (UpdateTrajectoryWithCurrentBehavior() != kSuccess) {
                ...
                LOG(ERROR) << "[Ssc]fail: has " << qp_trajs_.size() << " traj, " << valid_behaviors_.size()
                           << " behaviors.";
                return kWrongStatus;
        }
...
ErrorType SscPlanner::RunQpOptimization() {
        ...
        for (int i = 0; i < static_cast<int>(cube_list.size()); i++) {
                ...
                start_constraints.push_back(Vecf<2>(ego_frenet_state_.vec_s[0], ego_frenet_state_.vec_dt[0]));
                ...
                start_constraints.push_back(Vecf<2>(ego_frenet_state_.vec_s[2], ego_frenet_state_.vec_dt[2]));
                ...
                if (spline_generator.GetBezierSplineUsingCorridor(
                            cube_list[i], start_constraints, end_constraints, ref_stamps, ref_points,
                            cfg_.planner_cfg().weight_proximity(), &bezier_spline) != kSuccess) {
                        if (is_lateral_independent_) {
                                ...
                                for (auto& cube : cube_list[i]) {
                                        LOG(ERROR) << std::fixed << std::setprecision(3) << "[Ssc] t: ["
                                                   ...
                                                   << cube.p_lb[0] << ", " << cube.p_ub[0] << "], y: [" << cube.p_lb[1]
                                                   << ", " << cube.p_ub[1] << "]";
                                }
                                ...
                                LOG(ERROR) << std::fixed << std::setprecision(4) << "[Ssc]End sd position ("
                                           << end_constraints[0](0) << ", " << end_constraints[0](1) << ")";
                                LOG(ERROR) << std::fixed << std::setprecision(4) << "[Ssc]End sd velocity ("
                                           << end_constraints[1](0) << ", " << end_constraints[1](1) << ")";
                                LOG(ERROR) << std::fixed << std::setprecision(4) << "[Ssc]End state stamp: "
                ...
                if (!is_lateral_independent_) {
                        primitive.Connect(
                                initial_frenet_state_, fs_vehicle_traj.back().frenet_state,
...
ErrorType SscPlanner::UpdateTrajectoryWithCurrentBehavior() {
        ...
        if (!find_exact_match_behavior && !find_candidate_behavior) return kWrongStatus;
        ...
        for (int i = 1; i < num_cubes; i++) {
                if (cubes[i - 1].t_ub != cubes[i].t_lb) {
                        ...
                        LOG(ERROR) << "[Ssc]Err - t: [" << cubes[i - 1].t_lb << ", " << cubes[i - 1].t_ub << "], x: ["
                                   << cubes[i - 1].p_lb[0] << ", " << cubes[i - 1].p_ub[0] << "], y: ["
                                   << cubes[i - 1].p_lb[1] << ", " << cubes[i - 1].p_ub[1] << "]";
                        LOG(ERROR) << "[Ssc]Err - t: [" << cubes[i].t_lb << ", " << cubes[i].t_ub << "], x: ["
                                   << cubes[i].p_lb[0] << ", " << cubes[i].p_ub[0] << "], y: [" << cubes[i].p_lb[1]
                                   << ", " << cubes[i].p_ub[1] << "]";
                        return kWrongStatus;
                }
        }
...
ErrorType SscPlanner::StateTransformForInputData() {
        ...
        // * Surrounding vehicle trajs from MPDM
        {
                for (int i = 0; i < surround_forward_trajs_.size(); ++i) {
                        for (auto it = surround_forward_trajs_[i].begin(); it != surround_forward_trajs_[i].end();
                             ++it) {
                                for (int k = 0; k < it->second.size(); ++k) {
                                        ...
                                        // vertices
                                        vec_E<Vec2f> v_vec;
                                        common::SemanticsUtils::GetVehicleVertices(it->second[k].param(), traj_state,
                                                                                   &v_vec);
                                        global_point_vec.insert(global_point_vec.end(), v_vec.begin(), v_vec.end());
                                }
                        }
        ...
        // * Surrounding vehicle trajs from MPDM
        {
                ...
                for (int j = 0; j < surround_forward_trajs_.size(); ++j) {
                        ...
                        for (auto it = surround_forward_trajs_[j].begin(); it != surround_forward_trajs_[j].end();
                             ++it) {
                                int v_id = it->first;
                                vec_E<common::FsVehicle> traj_fs;
                                for (int k = 0; k < it->second.size(); ++k) {
...
ErrorType SscPlanner::ValidateTrajectory(const FrenetTrajectory& traj) {
        ...
        for (const auto t : t_vec_xy) {
                ...
                if (fabs(state.curvature) > 0.33) {
                        LOG(ERROR) << "[Ssc][Validate]initial_state velocity " << initial_state_.velocity
                                   << " Curvature " << state.curvature << " invalid.";
                        return kWrongStatus;
                }
        }
        return kSuccess;
