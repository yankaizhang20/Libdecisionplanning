...
moodycamel::ReaderWriterQueue<semantic_map_manager::SemanticMapManager>* p_ctrl_input_smm_buff_{nullptr};
...
int BehaviorUpdateBallback(const semantic_map_manager::SemanticMapManager& smm) {
...
void PublishControl() {
        ...
        if (last_smm.GetSpeedLimit(last_smm.ego_vehicle().state(), last_smm.ego_behavior().ref_lane, &speed_limit) ==
            kSuccess) {
        ...
        last_smm.GetLeadingVehicleOnLane(last_smm.ego_behavior().ref_lane, desired_state,
                                         last_smm.surrounding_vehicles(), lat_range, &leading_vehicle,
                                         &distance_residual_ratio);
        if (planning::OnLaneForwardSimulation::PropagateOnce(common::StateTransformer(last_smm.ego_behavior().ref_lane),
        ...
        common::VehicleControlSignal ctrl(state);
        {
                ...
                vehicle_msgs::Encoder::GetRosControlSignalFromControlSignal(ctrl, ros::Time::now(), std::string("map"),
                                                                            &ctrl_msg);
