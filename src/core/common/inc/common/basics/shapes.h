...
class ShapeUtils {
    public:
         ...
         * @return ErrorType
         */
        static ErrorType GetPerpendicularAxesOfOrientedBoundingBox(const vec_E<Vecf<2>>& vertices,
                                                                   vec_E<Vecf<2>>* axes);
        ...
        static bool CheckIfAxisAlignedCubeCollide(const common::AxisAlignedCubeNd<T, N_DIM>& aabb_a,
                ...
                for (int i = 0; i < N_DIM; ++i) {
                        decimal_t half_len_a =
                                fabs(static_cast<double>(aabb_a.upper_bound[i] - aabb_a.lower_bound[i]) / 2.0);
                        decimal_t half_len_b =
                                fabs(static_cast<double>(aabb_b.upper_bound[i] - aabb_b.lower_bound[i]) / 2.0);
