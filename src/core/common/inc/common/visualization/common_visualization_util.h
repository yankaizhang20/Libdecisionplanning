...
class VisualizationUtil {
    public:
         ...
         * @return ErrorType
         */
        static ErrorType GetMarkerByLane(const Lane& lane, const decimal_t step, const Vec3f& scale,
                                         const ColorARGB& color, const decimal_t offset_z,
                                         visualization_msgs::Marker* marker) {
         ...
         */
        template <int N_DIM>
        static ErrorType ConvertVectorToPoint(const Vecf<N_DIM> vec, geometry_msgs::Point* point) {
                point->x = 0.0;
                point->y = 0.0;
                point->z = 0.0;
         ...
         * @return ErrorType
         */
        static ErrorType FillStampInMarkerArray(const ros::Time time_stamp,
                                                visualization_msgs::MarkerArray* marker_arr) {
                for (auto& mk : marker_arr->markers) {
                        mk.header.stamp = time_stamp;
                }
        ...
        static ErrorType GetRosMarkerLineStripUsing2DofVecWithOffsetZ(const vec_E<Vec2f>& path, const ColorARGB& color,
                                                                      const Vec3f& scale, const decimal_t& z,
                                                                      const int& id,
                                                                      visualization_msgs::Marker* p_marker) {
                p_marker->type = visualization_msgs::Marker::LINE_STRIP;
                p_marker->action = visualization_msgs::Marker::MODIFY;
         ...
         * @return ErrorType
         */
        static ErrorType GetRosMarkerLineStripUsing3DofStateVec(const std::vector<Vec3f>& path,
                                                                const decimal_t z_offset, const ColorARGB& color,
                                                                const Vec3f& scale, const int& id,
                                                                visualization_msgs::Marker* p_marker) {
                p_marker->type = visualization_msgs::Marker::LINE_STRIP;
                p_marker->action = visualization_msgs::Marker::MODIFY;
        ...
        static ErrorType GetRosMarkerTextUsingPositionAndString(const Vec3f& pos, const std::string& str,
                                                                const ColorARGB& color, const Vec3f& scale,
                                                                const int& id, visualization_msgs::Marker* p_marker) {
         ...
         * @return ErrorType
         */
        static ErrorType GetRosMarkerUsingCircleObstacle(const CircleObstacle& obs,
                                                         visualization_msgs::Marker* p_marker) {
                p_marker->header.frame_id = "map";
                p_marker->header.stamp = ros::Time::now();
                        ...
                        if (behavior.ref_lane.IsValid()) {
                                ...
                                if (behavior.lat_behavior == common::LateralBehavior::kLaneChangeLeft ||
                                    behavior.lat_behavior == common::LateralBehavior::kLaneChangeRight) {
                                        FillScaleColorInMarker(Vec3f(0.4, 0.0, 0.0), ColorARGB(1.0, 1.0, 1.0, 0.0),
                                                               &direction_mk);
                                        angle = kPi / 4.0;
                                } else if (behavior.lon_behavior == common::LongitudinalBehavior::kDecelerate) {
                                        FillScaleColorInMarker(Vec3f(0.4, 0.0, 0.0), ColorARGB(1.0, 1.0, 0.0, 0.0),
                                                               &direction_mk);
                                        angle = 0.0;
                                } else {
                                        FillScaleColorInMarker(Vec3f(0.4, 0.0, 0.0), ColorARGB(1.0, 0.1, 0.8, 0.1),
                                                               &direction_mk);
                                        angle = kPi / 4.0;
                                }
                                ...
                                for (decimal_t s = behavior.ref_lane.begin(); s < behavior.ref_lane.end();
                                     s += sample_step) {
                                        Vecf<2> pos;
         ...
         */
        template <typename T>
        static ErrorType GetRosMarkerCubeListUsingGripMap3D(const GridMapND<T, 3>* p_grid_map,
