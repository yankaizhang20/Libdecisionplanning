...
class OnLaneForwardSimulation {
    public:
        ...
        static ErrorType GetTargetStateOnTargetLane(const common::StateTransformer& stf_target,
                ...
                if (gap_front_vehicle.id() != -1 &&
                    kSuccess == stf_target.GetFrenetStateFromState(gap_front_vehicle.state(), &front_fs)) {
                        has_front = true;
                        s_ref_front = front_fs.vec_s[0] -
                                      (gap_front_vehicle.param().length() / 2.0 - gap_front_vehicle.param().d_cr());
                ...
                decimal_t ego_desired_vel = ego_vehicle.state().velocity +
                                            (param.idm_param.kDesiredVelocity - ego_vehicle.state().velocity) * p_v_ego;
                if (has_front && has_rear) {
                        ...
                        decimal_t v_ref_front = std::max(
                                0.0, gap_front_vehicle.state().velocity + truncate(s_err_front * k_v, dv_lb, dv_ub));

                        decimal_t s_err_rear = s_thres_rear - ego_fs.vec_s[0];
                        decimal_t v_ref_rear = std::max(
                                0.0, gap_rear_vehicle.state().velocity + truncate(s_err_rear * k_v, dv_lb, dv_ub));
        ...
        static ErrorType PropagateOnceAdvancedLK(const common::StateTransformer& stf,
                                                 const common::Vehicle& ego_vehicle, const Vehicle& leading_vehicle,
                                                 const decimal_t& lat_track_offset, const decimal_t& dt,
                                                 const Param& param, State* desired_state) {
                ...
                if (!steer_calculation_failed) {
                        ...
                        if (CalcualateSteer(stf, current_state, current_fs, wheelbase_len,
                                            Vec2f(approx_lookahead_dist, lat_track_offset), &steer) != kSuccess) {
                                steer_calculation_failed = true;
                        }
                }
        ...
        static ErrorType PropagateOnceAdvancedLC(const common::StateTransformer& stf_current,
                                                 ...
                                                 const common::Vehicle& ego_vehicle,
                                                 const Vehicle& current_leading_vehicle,
                ...
                common::State target_state;
                if (kSuccess != GetTargetStateOnTargetLane(stf_target, ego_vehicle, gap_front_vehicle, gap_rear_vehicle,
                                                           sim_param, &target_state)) {
                ...
                if (current_leading_vehicle.id() == kInvalidAgentId ||
                    stf_current.GetFrenetStateFromState(current_leading_vehicle.state(), &current_leading_fs) !=
                            kSuccess) {
                        // ~ Without leading vehicle
                        CalcualateVelocityUsingCtxIdm(ego_on_tarlane_fs.vec_s[0], current_state.velocity,
                                                      target_on_curlane_fs.vec_s[0], target_state.velocity, dt,
                        ...
                        // * between ego vehicle and the leading vehicle.
                        decimal_t eqv_vehicle_len;
                        GetIdmEquivalentVehicleLength(stf_current, ego_vehicle, current_leading_vehicle,
                                                      current_leading_fs, &eqv_vehicle_len);
                        sim_param.idm_param.kVehicleLength = eqv_vehicle_len;

                        CalcualateVelocityUsingCtxIdm(
                                ego_on_tarlane_fs.vec_s[0], current_state.velocity, current_leading_fs.vec_s[0],
                                current_leading_vehicle.state().velocity, target_on_curlane_fs.vec_s[0],
                                target_state.velocity, dt, sim_param, ctx_param, &velocity);
                }
                ...
                if (!steer_calculation_failed) {
                        decimal_t approx_lookahead_dist =
                                std::min(std::max(param.steer_control_min_lookahead_dist,
                                         ...
                                         param.steer_control_max_lookahead_dist);
                        if (CalcualateSteer(stf, current_state, current_fs, wheelbase_len,
                                            Vec2f(approx_lookahead_dist, 0.0), &steer) != kSuccess) {
                                steer_calculation_failed = true;
                        }
                }
    ...
    private:
        static ErrorType GetIdmEquivalentVehicleLength(const common::StateTransformer& stf,
                                                       ...
                                                       const common::FrenetState& leading_fs,
                                                       decimal_t* eqv_vehicle_len) {
        ...
        static ErrorType CalcualateSteer(const common::StateTransformer& stf, const State& current_state,
                ...
                decimal_t look_ahead_dist = (dest_state.vec_position - current_state.vec_position).norm();
                decimal_t cur_to_dest_angle = vec2d_to_angle(dest_state.vec_position - current_state.vec_position);
                decimal_t angle_diff = normalize_angle(cur_to_dest_angle - current_state.angle);
        ...
        static ErrorType CalcualateVelocityUsingIdm(const decimal_t& current_vel, const decimal_t& dt,
                                                    const Param& param, decimal_t* velocity) {
        ...
        static ErrorType CalcualateVelocityUsingCtxIdm(const decimal_t& current_pos, const decimal_t& current_vel,
                                                       ...
                                                       const CtxParam& ctx_param, decimal_t* velocity) {
                decimal_t leading_vel_fin = leading_vel;
                if (leading_vel < 0) {
                        leading_vel_fin = 0;
                }
                ...
                return control::ContextIntelligentVelocityControl::CalculateDesiredVelocity(
                        param.idm_param, ctx_param, current_pos, leading_pos, target_pos, current_vel, leading_vel_fin,
        ...
        static ErrorType CalcualateVelocityUsingCtxIdm(const decimal_t& current_pos, const decimal_t& current_vel,
                ...
                return control::ContextIntelligentVelocityControl::CalculateDesiredVelocity(
                        param.idm_param, ctx_param, current_pos, virtual_leading_pos, target_pos, current_vel,
                        current_vel, target_vel, dt, velocity);
        }

        static ErrorType CalculateDesiredState(const State& current_state, const decimal_t steer,
                                               const decimal_t velocity, const decimal_t wheelbase_len,
                                               const decimal_t dt, const Param& param, State* state) {
                simulator::IdealSteerModel model(wheelbase_len, param.idm_param.kAcceleration,
