...
ErrorType DataRenderer::InjectObservationNoise() {
        ...
        if (cnt_random_ == 10) {
                ...
                for (auto &v : surrounding_vehicles_.vehicles) {
                        if (std::find(sampled_ids.begin(), sampled_ids.end(), v.first) == sampled_ids.end()) continue;
                        ...
                        Vec2f augmented_position = Vec2f(original_position.x() + lateral_position_noise * sin(angle) +
...
ErrorType DataRenderer::RayCastingOnObstacleMap() {
        ...
        for (int i = 0; i < 8; ++i) {
                ...
                roguelike_ray_casting::RasterizeFOVOctant(
                        coord[0], coord[1], r_idx_max, p_obstacle_grid_->dims_size(0), p_obstacle_grid_->dims_size(1),
                        i, p_obstacle_grid_->data_ptr(), render_mat.data(), &obs_coord_set);
