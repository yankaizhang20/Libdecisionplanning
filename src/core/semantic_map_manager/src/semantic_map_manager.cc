...
ErrorType SemanticMapManager::SaveMapToLog() {
        ...
        for (auto &v : vehicle_set.vehicles) {
                record_file_stream << v.second.state().time_stamp << "," << v.first << ","
                                   << v.second.state().vec_position[0] << "," << v.second.state().vec_position[1] << ","
                                   << v.second.state().velocity << "," << v.second.state().acceleration << ","
                                   << v.second.state().angle << "," << v.second.state().curvature << ","
...
ErrorType SemanticMapManager::MobilRuleBasedBehaviorPrediction(const common::Vehicle &vehicle,
        ...
        for (const auto &behavior : behaviors) {
                ...
                GetRefLaneForStateByBehavior(vehicle.state(), std::vector<int>(), behavior, lane_radius, lane_radius,
                                             false, &ref_lane);
                ...
                GetLeadingAndFollowingVehiclesFrenetStateOnLane(
                        ref_lane, vehicle.state(), nearby_vehicles, &has_leading_vehicle, &leading_vehicle,
        ...
        if (common::MobilBehaviorPrediction::LateralBehaviorPrediction(
                    vehicle, lanes, leading_vehicles, leading_frenet_states, following_vehicles, follow_frenet_states,
                    nearby_vehicles, res) != kSuccess) {
                return kWrongStatus;
        }
        return kSuccess;
}
...
ErrorType SemanticMapManager::GetEgoNearestLaneId(int *ego_lane_id) const {
        ...
        if (GetNearestLaneIdUsingState(ego_vehicle_.state().ToXYTheta(), std::vector<int>(), &nearest_lane_id,
...
ErrorType SemanticMapManager::OpenloopTrajectoryPrediction() {
        ...
        for (const auto &p_sv : semantic_surrounding_vehicles_.semantic_vehicles) {
                ...
                TrajectoryPredictionForVehicle(semantic_vehicle.vehicle, semantic_vehicle.lane, pred_time_, pred_step_,
                                               &traj);
...
ErrorType SemanticMapManager::UpdateLocalLanesAndFastLut() {
        ...
        if (kSuccess != GetNearestLaneIdUsingState(ego_state.ToXYTheta(), std::vector<int>(), &cur_lane_id, &dist_tmp,
                                                   &arc_len_tmp)) {
                return kWrongStatus;
        }
        ...
        for (const auto root_id : root_lane_ids) {
                decimal_t arc_len;
                semantic_lane_set_.semantic_lanes.at(root_id).lane.GetArcLengthByVecPosition(ego_state.vec_position,
                                                                                             &arc_len);
                ...
                for (const auto &path : assembled_paths) {
                        common::Lane lane;
                        if (kSuccess != GetLocalLaneUsingLaneIds(ego_state, path, local_lane_length_forward_,
...
void SemanticMapManager::GetAllForwardLaneIdPathsWithMinimumLengthByRecursion(
        ...
        if (node_aggre_length >= local_lane_length_forward_ || whole_lane_net_.lane_set.at(node_id).child_id.empty()) {
                ...
                for (const auto child_id : child_ids) {
                        ...
                        GetAllForwardLaneIdPathsWithMinimumLengthByRecursion(child_id, child_length, node_aggre_length,
                                                                             path, all_paths);
                }
        }
}
        ...
        if (node_aggre_length >= local_lane_length_backward_ ||
            whole_lane_net_.lane_set.at(node_id).father_id.empty()) {
                ...
                for (const auto father_id : father_ids) {
                        ...
                        GetAllBackwardLaneIdPathsWithMinimumLengthByRecursion(father_id, father_length,
                                                                              node_aggre_length, path, all_paths);
                }
        }
}
...
ErrorType SemanticMapManager::UpdateKeyVehicles() {
        ...
        if (GetNearestLaneIdUsingState(ego_vehicle_.state().ToXYTheta(), std::vector<int>(), &cur_lane_id,
                        ...
                        if (it != key_lane_ids.end()) {
                                ...
                                if (dist >= 0 && fabs(dist) < front_range) {
                                        ...
                                        key_vehicles_.vehicles.insert(std::pair<int, common::Vehicle>(
                                                v_id,
                                                semantic_surrounding_vehicles_.semantic_vehicles.at(v_id).vehicle));
                                }
                                // * Rear
...
ErrorType SemanticMapManager::GetRefLaneForStateByBehavior(const common::State &state,
        ...
        decimal_t arc_len;
        if (GetNearestLaneIdUsingState(state_3dof, navi_path, &current_lane_id, &distance_to_lane, &arc_len) !=
            kSuccess) {
