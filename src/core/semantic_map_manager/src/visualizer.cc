...
Visualizer::Visualizer(ros::NodeHandle nh, int node_id) : nh_(nh), node_id_(node_id) {
        ...
        std::string speed_limit_topic =
                std::string("/vis/agent_") + std::to_string(node_id_) + std::string("/speed_limit");
...
void Visualizer::VisualizeSurroundingVehicles(const ros::Time &stamp, const common::VehicleSet &vehicle_set,
        ...
        for (const auto &v : vehicle_set.vehicles) {
                ...
                common::VisualizationUtil::GetRosMarkerArrayUsingVehicle(v.second, color_obb, color_vel_vec,
                                                                         color_steer, 1, &vehicle_marker);
                for (auto &marker : vehicle_marker.markers) vehicle_marker_list.markers.push_back(marker);
        }
        int num_markers = static_cast<int>(vehicle_marker_list.markers.size());
        common::VisualizationUtil::FillHeaderIdInMarkerArray(
                stamp, std::string("map"), last_surrounding_vehicle_marker_cnt_, &vehicle_marker_list);
...
void Visualizer::VisualizeSurroundingLaneNet(const ros::Time &stamp, const common::LaneNet &lane_net,
        ...
        for (auto iter = lane_net.lane_set.begin(); iter != lane_net.lane_set.end(); ++iter) {
                if (deleted_lane_ids.end() !=
                    std::find(deleted_lane_ids.begin(), deleted_lane_ids.end(), iter->second.id)) {
                        continue;
                }
                ...
                common::VisualizationUtil::GetRosMarkerLineStripUsing2DofVec(
                        iter->second.lane_points, common::cmap.at("sky blue"), Vec3f(0.1, 0.1, 0.1), iter->second.id,
                        &lane_marker);
                ...
                visualization_msgs::Marker start_point_marker, end_point_marker, lane_id_text_marker;
                {
                        ...
                        common::VisualizationUtil::GetRosMarkerSphereUsingPoint(
                                Vec3f(pt(0), pt(1), 0.0), common::ColorARGB(1.0, 0.2, 0.6, 1.0), Vec3f(0.5, 0.5, 0.5),
                                id_cnt++, &start_point_marker);
...
void Visualizer::VisualizeSpeedLimit(const ros::Time &stamp, const vec_E<common::SpeedLimit> &speed_limits) {
        ...
        for (int i = 0; i < static_cast<int>(speed_limits.size()); ++i) {
                ...
                str_start += std::string(
                        common::GetStringByValueWithPrecision<decimal_t>(speed_limits[i].vel_range()(1), 1) + "m/s");
                common::VisualizationUtil::GetRosMarkerTextUsingPositionAndString(
                        Vec3f(start_x, start_y, 3.5), str_start, common::cmap.at("black"), Vec3f(0.75, 0.75, 0.75),
                        ++id_cnt, &start_text_marker);
...
void Visualizer::VisualizeIntentionPrediction(const ros::Time &stamp,
        ...
        common::VisualizationUtil::FillHeaderIdInMarkerArray(stamp, std::string("map"), last_intention_marker_cnt_,
                                                             &mks);
        ...
        for (const auto &p_traj : openloop_pred_trajs) {
                ...
                for (const auto &ps : p_traj.second) {
                        ...
                        common::VisualizationUtil::GetRosMarkerCylinderUsingPoint(
                                common::Point(pt), Vec3f(0.5, 0.5, 0.1), common::cmap.at("sky blue"), 0, &point_marker);
                        mks.markers.push_back(point_marker);
                }
                visualization_msgs::Marker line_marker;
                common::VisualizationUtil::GetRosMarkerLineStripUsingPoints(
                        points, Vec3f(0.1, 0.1, 0.1), common::cmap.at("sky blue"), 0, &line_marker);
                mks.markers.push_back(line_marker);
        }
        int num_mks = static_cast<int>(mks.markers.size());
        common::VisualizationUtil::FillHeaderIdInMarkerArray(stamp, std::string("map"), last_traj_list_marker_cnt_,
                                                             &mks);
