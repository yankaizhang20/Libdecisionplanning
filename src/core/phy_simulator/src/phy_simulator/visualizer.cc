...
Visualizer::Visualizer(ros::NodeHandle nh) : nh_(nh) {
        ...
        obstacle_set_pub_ = nh_.advertise<visualization_msgs::MarkerArray>("vis/obstacle_set_vis", 10);
}
...
void Visualizer::VisualizeVehicleSet(const ros::Time &stamp, const common::VehicleSet &vehicle_set) {
        ...
        for (auto iter = vehicle_set.vehicles.begin(); iter != vehicle_set.vehicles.end(); ++iter) {
                common::VisualizationUtil::GetRosMarkerArrayUsingVehicle(iter->second, common::ColorARGB(1, 1, 0, 0),
                                                                         color_vel_vec, color_steer, 7 * iter->first,
                                                                         &vehicle_marker);
        }
...
void Visualizer::VisualizeObstacleSet(const ros::Time &stamp, const common::ObstacleSet &Obstacle_set) {
        ...
        common::VisualizationUtil::GetRosMarkerUsingObstacleSet(Obstacle_set, &Obstacles_marker);
