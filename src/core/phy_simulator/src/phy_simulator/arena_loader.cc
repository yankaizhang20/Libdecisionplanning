...
ArenaLoader::ArenaLoader(const std::string &vehicle_set_path, const std::string &map_path,
...
ErrorType ArenaLoader::ParseMapInfo(common::ObstacleSet *p_obstacle_set) {
        ...
        for (int i = 0; i < static_cast<int>(obstacles_json.size()); ++i) {
                ...
                p_obstacle_set->obs_polygon.insert(std::pair<int, common::PolygonObstacle>(poly.id, poly));
        }
...
ErrorType ArenaLoader::ParseLaneNetInfo(common::LaneNet *p_lane_net) {
        ...
        for (int i = 0; i < static_cast<int>(lane_net_json.size()); ++i) {
                ...
                for (int k = 0; k < num_pts; ++k) {
                        Vec2f pt(lane_coordinates[k][0].get<double>(), lane_coordinates[k][1].get<double>());
