...
int main(int argc, char** argv) {
        ...
        while (ros::ok()) {
                ...
                if (tnow >= next_gt_pub_time) {
                        next_gt_pub_time += ros::Duration(1.0 / gt_msg_rate);  //动态目标消息发布速率为100HZ
                        ros_adapter.PublishDynamicDataWithStamp(tnow);
                }

                if (tnow >= next_gt_static_pub_time) {
                        next_gt_static_pub_time += ros::Duration(1.0 / gt_static_msg_rate); //静态目标消息发布速率为10HZ
                        ros_adapter.PublishStaticDataWithStamp(tnow);
                }

                //可视化
                if (tnow >= next_vis_pub_time) {
                        next_vis_pub_time += ros::Duration(1.0 / visualization_msg_rate);  //可视化频率为20HZ
