...
void BehaviorPlannerServer::PublishData() { p_visualizer_->PublishDataWithStamp(ros::Time::now()); }
...
void BehaviorPlannerServer::JoyCallback(const sensor_msgs::Joy::ConstPtr& msg) {
        ...
        // ~ buttons[3] --> 1 -->  +1m/s
        // ~ buttons[0] --> 1 -->  -1m/s
        if (msg->buttons[0] == 0 && msg->buttons[1] == 0 && msg->buttons[2] == 0 && msg->buttons[3] == 0) return;
...
void BehaviorPlannerServer::set_autonomous_level(int level) { bp_.set_autonomous_level(level); }

void BehaviorPlannerServer::set_aggressive_level(int level) { bp_.set_aggressive_level(level); }
...
decimal_t BehaviorPlannerServer::user_desired_velocity() const { return bp_.user_desired_velocity(); }

decimal_t BehaviorPlannerServer::reference_desired_velocity() const { return bp_.reference_desired_velocity(); }
